...
from .utils.dataset import SquadV1Processor, SquadV2Processor,unqoverGProcessor,unqoverRProcessor,unqoverEProcessor,unqoverNProcessor
...
@cli.command()
@click.option("--checkpoint-file", type=click.Path(exists=True),default='pytorch_model_reproduce.bin')
...
@click.option("--n-best-size", default=40)
...
@click.option("--do-unqover/--no-unqover", default=False)
@click.option("--unqover-file", default='n')
@trainer_args
@click.pass_obj
def run(common_args, **task_args):
    ...
    if args.do_eval:
        ...
        model.to(args.device)
        if args.unqover_file=="G":
            prefix='Gender_'
        elif args.unqover_file=="R":
            prefix='Religion_'
        elif args.unqover_file=="N":
            prefix='Nationality_'
        elif args.unqover_file=="E":
            prefix='Ethnicity_'
        else:
            prefix=''
    ...
    if args.do_unqover:
        return '***** unqover prediction finished *****'
    else:
...
def load_examples(args, evaluate=False):
    ...
    if args.do_unqover:
        if args.unqover_file == 'n':
            assert False,'No unqover file indicated'
        elif args.unqover_file == 'G':
            processor = unqoverGProcessor()
        elif args.unqover_file == 'R':
            processor = unqoverRProcessor()
        elif args.unqover_file == 'N':
            processor = unqoverNProcessor()
        elif args.unqover_file == 'E':
            processor = unqoverEProcessor()
    else:
        if args.with_negative:
